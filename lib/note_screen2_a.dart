...
class _NoteScreenState extends State<NoteScreen2> {
  ...
  void initState() {
    ...
    _ttsService.initialize().then((success) {
      if (!success && mounted) {
        _promptForTtsEngine();
      }
    });
  ...
  // Prompt user to install TTS engine if unavailable
  void _promptForTtsEngine() {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('TTS Engine Required'),
        content: const Text(
          'No text-to-speech engine is installed. Please install Google TTS or another TTS engine from the Play Store.',
        ),
        actions: [
          TextButton(
  ...
  Future<List<Map<String, dynamic>>> _fetchNotesWithMedia() async {
    return await compute(_fetchNotesWithMediaIsolate, {
      'searchQuery': _searchQuery,
      'sortOrder': _selectedSort.sqlOrder,
    });
  }

  static Future<List<Map<String, dynamic>>> _fetchNotesWithMediaIsolate(Map<String, dynamic> params) async {
    final dbHelper = DatabaseHelper2.instance;
    final db = await dbHelper.database;
    final searchQuery = params['searchQuery'] as String;
    final sortOrder = params['sortOrder'] as String;
    ...
    final query = '''
      ...
      ${searchQuery.isNotEmpty ? 'WHERE Note.title LIKE ?' : ''}
      ORDER BY $sortOrder
    ''';
  ...
  Future<void> _speakNote(Map<String, dynamic> note) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to read note aloud: $e')),
      );
    }
  }
...
class _NoteEditorScreenState extends State<NoteEditorScreen> {
  ...
  Future<void> _speakSelectedText(TextEditingController controller) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to read text aloud: $e')),
      );
    }
  }
        ...
        actions: [
          IconButton(
            ...
            onPressed: _ttsService.isSpeaking
                ...
                : () async {
              try {
                await _ttsService.speak('Title: ${_titleController.text}. Description: ${_descController.text}');
              } catch (e) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('Failed to read note aloud: $e')),
                );
